resource "azurerm_linux_virtual_machine" "vm_atividade_db" {
    name                  = "vmatividadedb"
    ...
    boot_diagnostics {
        storage_account_uri = azurerm_storage_account.st_atividade_db.primary_blob_endpoint
    }
    ...
    depends_on = [ azurerm_resource_group.rg_atividade_db, azurerm_network_interface.nic_atividade_db, azurerm_storage_account.st_atividade_db, azurerm_public_ip.publicip_atividade_db ]
}

resource "time_sleep" "wait_30_seconds_db" {
  depends_on = [azurerm_linux_virtual_machine.vm_atividade_db]
  create_duration = "30s"
}
